...
public class RequestHandlerMapBeanFactory {
     ...
     * @param schemaBasePath 校验文件或者文件夹的路径
     * @return 返回一个RequestHandlerValidate实例
     */
    public static RequestHandlerValidate newInstance(HandlerMethod method, RequestMappingInfo requestMappingInfo, String schemaBasePath){
        if(schemaBasePath.endsWith("yml")){
            try {
                if(!cacheMap.containsKey(schemaBasePath)){
                    cacheMap.put(schemaBasePath, ConvertYmlToSchema.convert(schemaBasePath));
                }
